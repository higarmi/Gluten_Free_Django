...
def about(request):
	html = "<html><body>Gluten-free project developed by Hector Garcia</body></html>"
...
def contact(request):
    if request.method=='POST':
        ...
        if formA.is_valid():
            titulo = 'message from our Recipe Center'
            contenido = formA.cleaned_data['message'] + "\n"
            contenido += 'Connect with: ' + formA.cleaned_data['email']
            email = EmailMessage(titulo, contenido, to=['johndoe@email.com'])
            email.send()
